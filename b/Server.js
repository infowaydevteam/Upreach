...
app.post('/api/triggerReplySms', async (req, res) => {
  ...
  if (!phone || !message || !campaignId || !userEmail) {
    ...
    return res.status(400).json({ message: 'Phone, message, campaign ID, and user email are required.' });
  }
  ...
  try {
    ...
    const contactResult = await client.query(contactQuery, [cleanedPhone, phone]);
    ...
    res.status(200).json({ success: true, message: 'Reply SMS sent successfully.' });
  } catch (err) {

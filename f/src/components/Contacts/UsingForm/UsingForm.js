...
const UsingForm = () => {
  ...
  useEffect(() => {
    const fetchContacts = async () => {
      if (!selectedCustomerName) {
        return; // Don't fetch contacts if no customer is selected
      }
      ...
      try {
        ...
        if (response.ok) {
          setContactList(data.data); // Set the fetched contact list
        } else {
    ...
    fetchContacts();
  }, [selectedCustomerName]); // This effect depends on selectedCustomerName
  ...
  const handleInputChange = (e) => {
   ...
   const handleCustomerChange = (e) => {
    ...
    setSelectedCustomerName(value); // Update selectedCustomerName on customer change
  };
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="form-group">
            ...
            >
              ...
              {loading ? (
                <option value="">Loading customers...</option> // Show loading message
              ) : error ? (
                <option value="">{`Error: ${error}`}</option> // Show error if failed to load
              ) : (
